...
export function SectionCard({ section, onUpdateTitle, isSelected = false, onSelectionChange }: SectionCardProps) {
  ...
  return (
    <Card className={cn("group transition-all", isSelected && "ring-2 ring-primary/50")} data-testid={`card-section-${section.id}`}>
      <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0 pb-3">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          {onSelectionChange && (
            <Checkbox
              checked={isSelected}
              onCheckedChange={(checked) => onSelectionChange(section.id, !!checked)}
              className="mt-1"
