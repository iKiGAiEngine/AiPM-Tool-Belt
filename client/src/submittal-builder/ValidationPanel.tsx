  ...
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      {items.slice(0, 20).map((item, i) => (
        <div key={i} style={{ fontSize: 11, color: "var(--text-secondary)", padding: "3px 0", borderBottom: "1px solid var(--border-ds)" }}>
          <span style={{ color: "var(--text-muted)" }}>{item.scope}</span>
          {item.line && <span style={{ color: "var(--text-muted)" }}> · {item.line}</span>}
          <span> — {item.msg}</span>
        </div>
      ))}
      {items.length > 20 && <div style={{ fontSize: 11, color: "var(--text-secondary)", paddingTop: 4 }}>+ {items.length - 20} more</div>}
    </div>
  );
}
...
export default function ValidationPanel({ project }: Props) {
  ...
  return (
    <div style={{ padding: 20, maxWidth: 780 }}>
      <div style={{ fontSize: 15, fontWeight: 700, color: "var(--text-primary)", marginBottom: 16 }}>Package Validation</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginBottom: 20 }}>
        {[
          ...
          { label: "Missing", value: s.missing, color: s.missing > 0 ? "var(--error)" : "var(--success)" },
        ].map((c, i) => (
          <div key={i} style={{ padding: 14, background: "var(--bg-card)", borderRadius: 8, border: "1px solid var(--border-ds)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginBottom: 20 }}>
        {[
          ...
          <div key={i} style={{ padding: 10, background: "var(--bg-card)", borderRadius: 6, border: "1px solid " + (c.warn ? "var(--error-border)" : "var(--border-ds)") }}>
            <div style={{ fontSize: 18, fontWeight: 700, color: c.warn ? "var(--error)" : "var(--text-primary)" }}>{c.value}</div>
      ...
      {v.errors.length === 0 && v.warnings.length === 0 && (
        <div style={{ padding: 24, textAlign: "center", color: "var(--success)", fontSize: 14, fontWeight: 600 }}>✓ All checks passed</div>
      )}
    </div>
  );
}
