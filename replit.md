...
- **Estimating Module Mobile Layout**: The Line Items stage renders mobile-friendly stacked layouts on viewports under 768px. The Add Item form stacks each field vertically with full-width inputs and labels above; the line items table is replaced by per-item cards showing description, plan callout, model, manufacturer, qty/uom/unit cost, quote dropdown, allocation expander, and inline action buttons (backup toggle, delete). Desktop continues to use the original 9-column form grid and table layout. Driven by the `useIsMobile` hook (`client/src/hooks/use-mobile.tsx`).
