...
- **Central Settings Hub**: Provides an administrative interface for managing scope dictionaries, regional identifiers, vendor profiles, Division 10 product dictionaries, and Spec Extractor configurations. Supports bulk import (Excel/CSV upload) for Vendors, Products, Scopes, and Regions with auto column mapping, preview, and duplicate detection.
...
- **BuildingConnected OAuth**: APS OAuth 2.0 integration allowing any authenticated user to connect their BuildingConnected account. Uses cryptographic nonce-based CSRF protection, per-user unique token storage in `aps_tokens` table, and automatic token refresh with concurrency-safe locking. Connect button visible to all users on Project Log page. Routes in `server/autodesk/auth.ts` and `server/autodesk/tokenManager.ts`.
- **BC Sync Engine**: Admin-only BuildingConnected opportunity sync with preview/confirm workflow. Fetches opportunities from BC API, filters by GC allowlist (Swinerton), maps location to regional codes (DEN, ATL, SFO, LAX, SEA, PDX, AUS, etc.), and creates draft proposal log entries. Supports 50-entry cap and 7-day lookback on first sync. Routes in `server/autodesk/bcSync.ts`. Draft entries have approve/reject admin actions.
...
- **Draft Review**: Proposal log entries can be marked as drafts (`isDraft` flag) via BC sync. Project Log page has tab-based filtering (All/Active/Drafts/Deleted) with DRAFT badges, scope pills, and admin-only approve/reject actions. Approved drafts clear `isDraft` flag; rejected drafts set `isDraft=false` and `deletedAt`.
