...
- **Inline Editable Columns**: Region (select dropdown from DB regions), GC Estimate Lead (text), Final Reviewer (text), Swinerton? (Y/N select), Primary Market (select from predefined list), and Invite Date (date picker) are all inline-editable in the Proposal Log table. `finalReviewer` and `swinertonProject` are stored in proposal_log_entries DB columns.
