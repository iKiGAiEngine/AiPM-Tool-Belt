...
- **SP Estimator Column**: Editable `selfPerformEstimator` field on each proposal log entry. Inline editing shows a dropdown of existing estimators from the matched region's `selfPerformEstimators` array (append-only list) plus a free-text input for new names. New names are automatically appended to the region's estimator list (case-insensitive dedup). Supports text filtering and CSV export. Auto-populated from region settings on new entry creation (manual, bulk import, BC sync).
